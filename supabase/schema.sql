...
-- User stamps (unlocked stamps per user)
...
CREATE INDEX IF NOT EXISTS idx_letters_delivers_at ON letters(delivers_at);
...
-- Deliver letters that have passed their delivery time
...
-- Letters: users can see their own letters (sent or received)
...
-- Stamps: everyone can read stamps
CREATE POLICY "stamps_select" ON stamps FOR SELECT USING (true);

-- User stamps: users can see their own, insert their own
CREATE POLICY "user_stamps_select" ON user_stamps FOR SELECT
    USING (auth.uid() = user_id);
CREATE POLICY "user_stamps_insert" ON user_stamps FOR INSERT
